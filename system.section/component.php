...
if ($this->StartResultCache($arParams["CACHE_TIME"], serialize($cacheID))){
    ...
    $arSelect = array(
        ...
        "DETAIL_PICTURE",
        "DETAIL_PAGE_URL",
        "PROPERTY_*" // Получаем все свойства
    );
    ...
    while($ob = $rsElements->GetNextElement()){
        ...
        if($arFields["PREVIEW_PICTURE"]){
            $arResult["ITEMS"][$arFields["ID"]]["PREVIEW_PICTURE"] = CFile::GetFileArray($arFields["PREVIEW_PICTURE"]);
        }
        if($arFields["DETAIL_PICTURE"]){
            $arResult["ITEMS"][$arFields["ID"]]["DETAIL_PICTURE"] = CFile::GetFileArray($arFields["DETAIL_PICTURE"]);
        }

        // Получаем связанные элементы из свойства SVYAZ
        if(!empty($arProps["SVYAZ"]["VALUE"])){
            $arResult["ITEMS"][$arFields["ID"]]["SVYAZ_ELEMENTS"] = array();
            $svyazElements = (array)$arProps["SVYAZ"]["VALUE"];
            
            foreach($svyazElements as $svyazElementId){
                $resSvyaz = CIBlockElement::GetByID($svyazElementId);
                if($obSvyaz = $resSvyaz->GetNextElement()){
                    $arSvyazFields = $obSvyaz->GetFields();
                    $arSvyazProps = $obSvyaz->GetProperties();
                    
                    // Обработка изображений связанных элементов
                    if($arSvyazFields["PREVIEW_PICTURE"]){
                        $arSvyazFields["PREVIEW_PICTURE"] = CFile::GetFileArray($arSvyazFields["PREVIEW_PICTURE"]);
                    }
                    if($arSvyazFields["DETAIL_PICTURE"]){
                        $arSvyazFields["DETAIL_PICTURE"] = CFile::GetFileArray($arSvyazFields["DETAIL_PICTURE"]);
                    }
                    
                    $arResult["ITEMS"][$arFields["ID"]]["SVYAZ_ELEMENTS"][] = array(
                        "FIELDS" => $arSvyazFields,
                        "PROPERTIES" => $arSvyazProps
                    );
                }
            }
        }
